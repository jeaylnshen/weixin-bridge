# Weixin Agent Bridge Client 0.3.3
...
- Node.js 22+
- One supported CLI: Codex, Claude, OpenCode, Google Antigravity (`agy`), or CodeBuddy (`codebuddy`)
...
Set `WEIXIN_AGENT_TYPE` to `codex`, `claude`, `opencode`, `agy`, or `codebuddy`. Explicit selection
...
For CodeBuddy, set `WEIXIN_AGENT_TYPE=codebuddy` and set `WEIXIN_AGENT_BIN`
if `codebuddy` is not on PATH. It supports `--print`, text output, and
respects approval policy mapping.
